...
export default function ShortsPage() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true); setUploadProg('영상 업로드 중...')

    const fd = new FormData()
    fd.append('file', videoFile)
    const upRes  = await fetch('/api/upload-video', { method: 'POST', body: fd })
    const upData = await upRes.json()
    if (!upData.url) { alert(upData.error || '업로드 실패'); setUploading(false); return }
    ...
    await fetch('/api/shorts', {
      ...
      body: JSON.stringify({
        ...
        videoUrl: upData.url, title, description: desc,
      }),
    })
    setVideoFile(null); setVideoPreview(null); setTitle(''); setDesc('')
    setUploading(false); setUploadProg(''); setShowUpload(false)
    loadShorts()
  }
