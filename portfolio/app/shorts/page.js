  ...
  return (
    <main className="s-bg">
        ...
        <div className="s-video-col">
          <div className="s-topbar">
            ...
            <div style={{display:'flex',gap:'0.5rem',alignItems:'center'}}>
              <button onClick={() => setMuted(m => !m)} className="s-icon-btn">{muted ? '🔇' : '🔊'}</button>
              {user
                    ...
                    return (
                      <div className="s-pc-item" onClick={e => handleTap(current, s.id, e)}>
                        {s.mediaType === 'image' || s.imageUrl
                          ...
                          : <video key={s.id} ref={el => { videoRefs.current[current] = el }} src={s.videoUrl} loop playsInline autoPlay className="s-video"/>
                        }
                        {heartPos?.idx === current && <div className="s-heart" style={{left:heartPos.x-40,top:heartPos.y-40}}>❤️</div>}
                  ...
                  {shorts.map((s, idx) => {
                    ...
                    return (
                      <div key={s.id} className="s-mobile-item"
                        ...
                        {s.mediaType === 'image' || s.imageUrl
                          ? <img src={s.imageUrl} alt={s.title||''} className="s-video" style={{objectFit:'contain',background:'#000'}}/>
                          : <video ref={el => { videoRefs.current[idx] = el }} src={s.videoUrl} loop playsInline className="s-video"/>
                        }
            ...
            {uploadType === 'image' && (
              imagePreview
                ...
                : <div className="upload-drop" onClick={() => imageFileRef.current?.click()}>
                    <div style={{fontSize:'2.2rem',marginBottom:'0.5rem'}}>🖼</div>
                    <p style={{fontFamily:'var(--mono)',fontSize:'0.82rem',color:'var(--muted)'}}>클릭하여 이미지 선택</p>
                    <p style={{fontFamily:'var(--mono)',fontSize:'0.68rem',color:'var(--border-dark)',marginTop:'0.25rem'}}>jpg · png · gif · webp · 5MB 이하</p>
                  </div>
            )}
      ...
      <style>{`
        ...
        .s-topbar { position:absolute; top:0; left:0; right:0; height:50px; z-index:200; background:rgba(10,10,10,.92); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:space-between; padding:0 1.5rem; border-bottom:1px solid rgba(255,255,255,.07); }
