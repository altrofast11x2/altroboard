...
export default function ShortsPage() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    setUploadProg('영상 업로드 중... (브라우저에서 직접 업로드)')
    setUploadErr('')

    try {
      // Step 1: 브라우저 → Vercel Blob 직접 업로드 (API 크기 제한 우회)
      ...
      const blob = await upload(filename, videoFile, {
        ...
        onUploadProgress: ({ percentage }) => {
          setUploadProg(`영상 업로드 중... ${Math.round(percentage)}%`)
        },
      })

      if (!blob?.url) {
        setUploadErr('영상 업로드 실패. 다시 시도해주세요.')
