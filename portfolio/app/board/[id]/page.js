    ...
    <main>
      <div className="container" style={{maxWidth:'780px'}}>
        ...
        <div className="card card-accent">
          ...
          {/* 🎵 음악 플레이어 */}
          {post.music && (
            <div style={{ marginBottom: '1.25rem', border: '1px solid var(--border)', borderRadius: '6px', overflow: 'hidden', background: 'var(--surface2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', borderBottom: '1px solid var(--border)' }}>
                <span style={{ fontSize: '1.1rem' }}>🎵</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: 'var(--serif)', fontSize: '0.88rem', fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{post.music.title}</div>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: '0.7rem', color: 'var(--muted)' }}>{post.music.author}</div>
                </div>
                <a href={post.music.url} target="_blank" rel="noopener noreferrer"
                  style={{ fontFamily: 'var(--mono)', fontSize: '0.68rem', color: 'var(--accent)', flexShrink: 0 }}>
                  SoundCloud ↗
                </a>
              </div>
              <div dangerouslySetInnerHTML={{ __html: post.music.html }} style={{ lineHeight: 0 }} />
            </div>
          )}
