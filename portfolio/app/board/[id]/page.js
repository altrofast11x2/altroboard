...
export default function PostPage({ params }) {
  ...
  const canDelete = user && (isStaff || (post && (post.authorId === user.id || post.authorId === user.email)))
    ...
    <main>
      <div className="container" style={{maxWidth:'780px'}}>
        ...
        <div className="card card-accent">
          ...
          <div style={{display:'flex',gap:'0.5rem',justifyContent:'flex-end',marginTop:'1rem',borderTop:'1px solid var(--border)',paddingTop:'1rem',flexWrap:'wrap'}}>
              {user && post.authorId !== user.id && (
                <ReportButton
                  type="post" targetId={postId}
                  targetUrl={typeof window !== 'undefined' ? `${window.location.origin}/board/${postId}` : ''}
                  targetAuthorId={post.authorId} targetAuthorName={post.author}
                />
              )}
          {(canEdit||canDelete) && (
            <>
              ...
              {canDelete && <button className="btn btn-danger btn-sm" onClick={handleDelete}>{['owner','admin'].includes(user?.role)?'👑 관리자 삭제':'삭제'}</button>}
            </>
          )}
          </div>
        </div>
