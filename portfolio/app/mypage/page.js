...
const TABS_EN = ['My Info', 'My Posts', 'Followers', 'Following', 'Language']
const TABS_KO = ['내 정보', '게시글', '팔로워', '팔로잉', '언어 설정']
    ...
    <main>
      <div className="container" style={{ maxWidth: '780px' }}>
        ...
        <div className="tab-row" style={{ marginBottom: '1rem' }}>
          {(lang==='ko' ? TABS_KO : TABS_EN).map((t, i) => (
        ...
        {tab === 4 && (
          <div className="card">
            <h3 style={{ fontFamily:'var(--serif)', marginBottom:'0.75rem', color:'var(--ink)' }}>
              {lang==='ko' ? '언어 설정' : 'Language Settings'}
            </h3>
            <p style={{ fontFamily:'var(--mono)', fontSize:'0.78rem', color:'var(--muted)', marginBottom:'1.25rem' }}>
              {lang==='ko' ? '사용할 언어를 선택해주세요. 새로고침 후 완전히 적용됩니다.' : 'Choose your preferred language. Reload page to apply fully.'}
            </p>
            <div style={{ display:'flex', gap:'0.75rem' }}>
              {[['en','🇺🇸 English'],['ko','🇰🇷 한국어']].map(([code, label]) => (
                <button key={code}
                  ...
                  onClick={() => {
                    localStorage.setItem('cozyboard_lang', code)
                    setLang(code)
                    setLangMsg(code==='ko' ? '언어 변경 중...' : 'Applying...')
            ...
            {langMsg && (
              <p style={{ fontFamily:'var(--mono)', fontSize:'0.75rem', color:'#27ae60', marginTop:'1rem' }}>✓ {langMsg}</p>
            )}
          </div>
        )}
