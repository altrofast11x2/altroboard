  ...
  return (
    <>
      <nav className="navbar">
        ...
        {user ? (
          <>
            <Link href="/mypage" className={`nav-user ${user.role==='admin'?'admin':''}`} style={{cursor:'pointer',display:'inline-flex',alignItems:'center',gap:'5px'}}>
              {user.avatar {user.role === 'admin' ? '👑 ' : ''}{user.name}{user.role === 'admin' ? '👑 ' : ''}{user.name} <img src={user.avatar} alt="" style={{width:20,height:20,borderRadius:'50%',objectFit:'cover'}} />}{user.role === 'admin' ? '👑 ' : ''}{user.name}
            </Link>
