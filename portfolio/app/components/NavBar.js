          ...
          <Link href="/chat" className="nav-link" style={{ position: 'relative' }}>
            ...
            {unread > 0 && (
              <span style={{
                ...
                padding: '0.05rem 0.35rem', minWidth: '16px', textAlign: 'center', lineHeight: '1.4'
            ...
            <Link href="/mypage" className={`nav-user ${user.role === 'admin' ? 'admin' : ''}`}>
              {user.avatar && (
                <img
                  src={user.avatar}
                  alt=""
                  style={{ width: 20, height: 20, borderRadius: '50%', objectFit: 'cover', marginRight: 4, verticalAlign: 'middle' }}
                />
              )}
