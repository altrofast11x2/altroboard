...
function ChatPageInner() {
  ...
  const loadRooms = async (parsedUser, openWithId, openWithName) => {
    setLoading(true)
    const res = await fetch(`/api/chat?userId=${parsedUser.id}`)
    const data = await res.json()
    setRooms(data)
    setLoading(false)

    if (openWithId) {
      const [a, b] = [parsedUser.id, openWithId].sort()
      const roomId = `${a}__${b}`
      const existing = data.find(r => r.roomId === roomId)
      // 기존 방이 있으면 그걸 열고, 없으면 임시 방 객체 생성
      openRoom(
        roomId,
      ...
      // ★ 버그 수정: 프로필에서 오지 않았을 때 첫 번째 방을 자동 선택하지 않음
      // 사용자가 직접 선택하도록 대기
    }
  }
