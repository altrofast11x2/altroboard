...
// POST: { followerId, followingId, action? } → toggle follow (기본) 또는 명시적 action
//   action='removeFollower'  → 본인의 팔로워 목록에서 followerId 제거 (= follows/{followerId}/{본인} 삭제)
//   action 없거나 'toggle'   → 기존 토글 동작
export async function POST(request) {
  ...
  // 팔로워 제거 — 본인(followingId)의 팔로워(followerId)를 끊음
  if (action === 'removeFollower') {
    await remove(ref(db, `follows/${followerId}/${followingId}`))
